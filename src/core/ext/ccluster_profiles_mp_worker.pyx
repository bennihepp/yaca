...
"""
ccluster_profiles_mp_worker.pyx -- Fast multiprocessing computation and
...
CCLUSTER_ID_CHUNK = 0
CCLUSTER_ID_TREATMENT = 1


def compute_treatment_distance_map_worker(
    queue,
    ...
    result = cy_compute_treatment_distance_map_worker(
        queue, start_index, stop_index, clusterProfiles,
        profile_metric_index,
        profile_threshold,
        binSimilarityMatrix,
        normalizationFactor)
    queue.put((CCLUSTER_ID_CHUNK, (result, start_index, stop_index)))
...
def cy_compute_treatment_distance_map_worker(
    ...
    #for i in prange(ntreatments, nogil=True):
        ...
        #sys.stdout.write('  \rcomputing profile #{} out of {}'.format(i + 1, clusterProfiles.shape[0]))
        #sys.stdout.flush()
        ...
        queue.put((CCLUSTER_ID_TREATMENT, i))

    #sys.stdout.write('\n')
    #sys.stdout.flush()

    return profileHeatmap[start_index:stop_index]
