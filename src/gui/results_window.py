...
class ResultsWindow(QMainWindow):
    ...
    def __init__(self, pipeline, channelMapping, channelDescription, simple_ui=False, parent=None):
        ...
        if self.group_policy == self.GROUP_POLICY_CLUSTERING:
            ...
            self.sorting_by_group = numpy.argsort(
                self.features[:, self.pdc.objTreatmentFeatureId])
        ...
        #for i in xrange(self.mahalFeatures.shape[1]):
            #featureDescription['mahal_' + self.featureNames[i]] \
                #= i + self.mahalFeatureIdOffset

        self.gallery = GalleryWindow(pipeline, featureDescription,
                                     self.channelMapping,
        ...
        self.__cell_mask_dict = {
            'All' : [numpy.ones((len(self.pdc.objects),), dtype=bool),
                     'All cells'],
            'Non-control-like' : [self.pipeline.nonControlCellMask,
                                  'Non-control-like cells'],
            'Control-like' : [self.pipeline.controlCellMask,
                              'Control-like cells'],
            'Valid' : [self.pipeline.validCellMask, 'All valid cells'],
            'Invalid' : [numpy.invert(self.pipeline.validCellMask),
                         'All invalid cells']
        }
    ...
    def on_view_cells(self, groupId=-1, focusId=-1, tabs=[]):
        ...
        caption = 'Showing cells from %s %s' % (self.get_group_policy_name(),
                                                self.get_group_name())
    ...
    def view_cells(self, selectionMask, focusId, caption, tabs=[]):
        ...
        selectionIds = self.pdc.objFeatures[:, self.pdc.objObjectFeatureId]\
            [selectionMask]

        #featureFactory = CellFeatureTextFactory(
            #self.pdc, self.pipeline.nonControlCellMask)
        #pixmapFactory = CellPixmapFactory(
            #self.pdc, self.channelMapping, self.pipeline.nonControlCellMask)
        ...
        self.gallery.on_selection_changed(focusObjId, selectionIds,
                                          pixmapFactory, featureFactory)
    ...
    def on_print_cell_population_and_penetrance(self, pdf_filename=None):
        ...
        print_engine.print_cell_populations_and_penetrance(self.pipeline,
                                                           pdf_filename)
        print 'done'
        ...
        if result:
            ...
            if self.__clustering_done:
                ...
                self.pipeline.disconnect(
                    self.pipeline, pipeline.SIGNAL('finished()'),
                ...
                self.pipeline.start_cluster_profiling(
                    self.cluster_method, self.hcluster_method,
                    self.cluster_exp_factor,
    ...
    def on_cluster_button(self):
        ...
        if not self.__pipeline_running:
            ...
            self.selected_groups = map(QListWidgetItem.text,
                                       self.groups_listwidget.selectedItems())
            ...
            self.cluster_method = str(self.cluster_method_combo.itemData(
                self.cluster_method_combo.currentIndex()).toString())
            self.hcluster_method = str(self.hcluster_method_combo.itemData(
                self.hcluster_method_combo.currentIndex()).toString())
            self.cluster_profile_metric = str(
                self.cluster_profile_metric_combo.itemData(
                    self.cluster_profile_metric_combo.currentIndex()
                ).toString()
            )
            #self.pipeline.run_clustering(method, self.featureset_index,
                                         #self.cluster_param1,
                                         #self.cluster_param2,
                                         #self.cluster_param3)

            self.pipeline.connect(self.pipeline, pipeline.SIGNAL('finished()'),
                                  self.on_pipeline_finished)
            ...
            self.pipeline.start_clustering(
                self.selected_groups[0], self.cluster_method,
                self.featureset_index,
    ...
    def on_group_policy_changed(self, group_policy):
        ...
        if self.group_policy == self.GROUP_POLICY_CLUSTERING:
            self.features = self.pdc.objFeatures[self.pipeline.nonControlCellMask]
            self.sorting_by_group = numpy.argsort(self.pipeline.partition)
        elif self.group_policy == self.GROUP_POLICY_TREATMENT:
            self.features = self.pdc.objFeatures[
                self.pipeline.nonControlCellMask]
            self.sorting_by_group = numpy.argsort(
                self.features[: , self.pdc.objTreatmentFeatureId])
    ...
    def on_view_all_cells(self):
        ...
        try:
            ...
            cell_mask_filter_widget = CellMaskFilterWidget(
                self.pipeline, self.gallery)

        self.view_cells(cell_mask, self.picked_index, caption,
                        [(cell_mask_filter_widget, 'Filter')])
    ...
    def build_widget(self):
        ...
        self.buttonGroup.addButton(treatmentRadiobutton,
                                   self.GROUP_POLICY_TREATMENT)
        self.buttonGroup.addButton(clusteringRadiobutton,
                                   self.GROUP_POLICY_CLUSTERING)
        ...
        self.connect(self.group_combo, SIGNAL('activated(int)'),
                     self.on_group_combo_activated)
        ...
        self.view_cells_button.setEnabled(False)
        self.connect(self.view_cells_button, SIGNAL('clicked()'),
                     self.on_view_cells)

        self.show_plots_button = QPushButton('Show plots')
        self.show_plots_button.setEnabled(False)
        self.connect(self.show_plots_button, SIGNAL('clicked()'),
                     self.on_show_plots)

        self.dissolve_cluster_button = QPushButton('Dissolve cluster')
        self.dissolve_cluster_button.setEnabled(False)
        self.connect(self.dissolve_cluster_button, SIGNAL('clicked()'),
                     self.on_dissolve_cluster)

        self.merge_cluster_button = QPushButton('Merge with cluster')
        self.merge_cluster_button.setEnabled(False)
        self.connect(self.merge_cluster_button, SIGNAL('clicked()'),
                     self.on_merge_cluster)

        self.merge_cluster_combo = QComboBox()
        self.merge_cluster_combo.setCurrentIndex(-1)
        self.merge_cluster_combo.setEnabled(False)
        self.connect(self.merge_cluster_combo, SIGNAL('activated(int)'),
                     self.on_merge_cluster_combo_activated)

        #self.print_cells_button = QPushButton('Print cell galleries')
        #self.connect(self.print_cells_button, SIGNAL('clicked()'),
                     #self.on_print_cells)

        #self.print_cluster_profiles_button \
            #= QPushButton('Print cluster profiles')
        #self.print_cluster_profiles_button.setEnabled(False)
        #self.connect(self.print_cluster_profiles_button, SIGNAL('clicked()'),
                     #self.on_print_cluster_profiles)
        ...
        self.connect(self.cluster_param1_spinbox, SIGNAL('valueChanged(int)'),
                     self.on_cluster_param1_changed)

        cluster_button = QPushButton('Run clustering')
        self.connect(cluster_button, SIGNAL('clicked()'),
                     self.on_cluster_button)
        ...
        self.connect(self.cluster_param2_spinbox, SIGNAL('valueChanged(int)'),
                     self.on_cluster_param2_changed)
        ...
        self.connect(self.cluster_param3_spinbox, SIGNAL('valueChanged(int)'),
                     self.on_cluster_param3_changed)
        ...
        self.connect(self.cluster_param4_spinbox, SIGNAL('valueChanged(int)'),
                     self.on_cluster_param4_changed)
        ...
        self.connect(self.cluster_exp_factor_spinbox,
                     SIGNAL('valueChanged(int)'),
                     self.on_cluster_exp_factor_changed)
        ...
        k = 0
        for i,(descr,name) \
            in enumerate(pipeline.Pipeline.get_hcluster_methods()):
        ...
        for descr,name in [['Quadratic Chi','quadratic_chi'],
                           ['L2 norm','l2_norm'],
                           ['Chi2 norm','chi2_norm'],
                           ['Summed MinMax','summed_minmax']]:
        ...
        self.featureset_index = 0
        self.connect(self.featureset_combo, SIGNAL('currentIndexChanged(int)'),
                     self.on_featureset_changed)
        hbox3 = QHBoxLayout()
        hbox3.addWidget(self.featureset_label)
        hbox3.addWidget(self.featureset_combo)

        self.groups_listwidget = QListWidget()
        self.groups_listwidget.setSelectionMode(
        ...
        #for i in xrange(self.mahalFeatures.shape[1]):
            #self.x_combo.addItem('mahalanobis_%s' % self.featureNames[i],
                                 #'mahal %d' % i)
            #self.y_combo.addItem('mahalanobis_%s' % self.featureNames[i],
                                 #'mahal %d' % i)
        ...
        self.connect(self.x_combo, SIGNAL('currentIndexChanged(int)'),
                     self.on_x_combo_changed)
        self.connect(self.y_combo, SIGNAL('currentIndexChanged(int)'),
        ...
        #for i in xrange(self.mahalFeatures.shape[1]):
            #self.histogram_combo.addItem(
                #'mahalanobis_%s' % self.featureNames[i],'mahal %d' % i)
        ...
        for name in keys:
            ...
            if featureId == self.pdc.objTreatmentFeatureId:
                self.histogram_combo.setCurrentIndex(
                    self.histogram_combo.count() - 1)

        self.connect(self.histogram_combo, SIGNAL('currentIndexChanged(int)'),
                     self.on_histogram_changed)

        self.histogram_button = QPushButton('Draw')
        self.histogram_button.setEnabled(False)
        self.connect(self.histogram_button, SIGNAL('clicked()'),
                     self.on_draw_histogram)
        ...
        self.connect(self.histogram_spinbox, SIGNAL('valueChanged(int)'),
                     self.on_histogram_changed)
        ...
        self.connect(self.__view_cell_mask_combo,
                     SIGNAL('currentIndexChanged(int)'),
                     self.on_view_cell_mask_changed)

        view_all_cells_button = QPushButton('View cells')
        self.connect(view_all_cells_button,
                     SIGNAL('clicked()'),
                     self.on_view_all_cells)
    ...
    def on_load_clusters(self):
        ...
        if path:
            ...
            self.statusBar().showMessage(
                'Loaded pipeline clusters from %s' % path)
            self.on_group_policy_changed(self.group_policy)
    ...
    def build_menu(self):
        ...
        self.add_actions(self.file_menu, 
            (open_state_action, save_state_action, None,
             open_clusters_action, save_clusters_action)
       )

        self.print_menu = self.menuBar().addMenu("&Print")
        population_and_penetrance_action = self.make_action(
            "Print cell population and penetrance",
            shortcut="Ctrl+Q",
            slot=self.on_print_cell_population_and_penetrance, 
        ...
        self.add_actions(self.print_menu, 
            (population_and_penetrance_action, selection_action, None,
              cluster_action, profile_action, norm0_profile_action,
              norm1_profile_action, norm2_profile_action, norm3_profile_action,
              norm4_profile_action, all_profile_action, gallery_action)
       )
