...
best_feature_mask = select_best_features(feature_quality, feature_mask, 5)
...
rnd_i = numpy.random.randint(0, numpy.sum(mask))

dist = distance.minkowski_dist(best_features[mask][rnd_i], best_features[mask])

best_i = numpy.argsort(dist)[0, : CLUSTER_EXPANSION]

best_mask = numpy.zeros((mask.shape[0],), dtype=bool)
best_mask[numpy.arange(mask.shape[0])[mask][rnd_i]] = True
best_mask[numpy.arange(mask.shape[0])[mask][best_i]] = True


caption = 'Showing control cells'
gallery_window.update_caption(caption)
focusObjId = pdc.objFeatures[:, pdc.objObjectFeatureId][mask][rnd_i]
selectionIds = pdc.objFeatures[: , pdc.objObjectFeatureId][best_mask]
gallery_window.on_selection_changed(focusObjId, selectionIds, pixmapFactory, featureFactory)
gallery_window.show()

def libsvm_oneclass(train_set, test_set):
    size_cache=10
    width=2.1
    C=10.
    epsilon=1e-5
    ...
    from sg import sg
    sg('set_features', 'TRAIN', train_set)
    ...
    sg('set_features', 'TEST', test_set)
    result=sg('classify')
    return result

train_set = best_features[best_mask].transpose()
test_set = best_features[mask].transpose()
result = libsvm_oneclass(train_set, test_set)
min_result_threshold = numpy.min(result[best_i])
print 'min_result_threshold:', min_result_threshold
match_mask = result >= min_result_threshold
print 'found %d matches' % numpy.sum(match_mask)

best_mask = numpy.zeros((mask.shape[0],), dtype=bool)
best_mask[numpy.arange(mask.shape[0])[mask][match_mask]] = True
...
gallery_window.update_caption(caption)
focusObjId = pdc.objFeatures[:, pdc.objObjectFeatureId][mask][rnd_i]
selectionIds = pdc.objFeatures[: , pdc.objObjectFeatureId][mask][match_mask]
gallery_window.on_selection_changed(focusObjId, selectionIds, pixmapFactory, featureFactory)
gallery_window.show()
